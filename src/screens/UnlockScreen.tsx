...
import { Star, Store, ChevronDown } from 'lucide-react-native';
...
export function UnlockScreen({ navigation }: NativeStackScreenProps<RootStackParamList, 'Unlock'>) {
    ...
    }, [sync, refreshSession, loadFavorites]));
    ...
    return (
        <Screen>
            ...
            <ScrollView contentContainerStyle={styles.grid}>
                ...
                {!ordered.length && <Text style={styles.empty}>Aucun serveur. Synchronisez la config.</Text>}
            </ScrollView>
        </Screen>
    );
}
...
const styles = StyleSheet.create({
    ...
    empty: { color: theme.colors.textMuted, textAlign: 'center', width: '100%', marginTop: theme.spacing(8) },
});
